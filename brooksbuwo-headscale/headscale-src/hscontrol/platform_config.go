...
// AndroidConfigMessage shows setup instructions for the Android Tailscale client.
func (h *Headscale) AndroidConfigMessage(
	writer http.ResponseWriter,
	req *http.Request,
) {
	writer.Header().Set("Content-Type", "text/html; charset=utf-8")
	writer.WriteHeader(http.StatusOK)
	_, _ = writer.Write([]byte(templates.Android(h.cfg.ServerURL).Render()))
}
